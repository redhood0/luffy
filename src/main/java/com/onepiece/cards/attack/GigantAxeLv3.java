...
public class GigantAxeLv3 extends CustomCard {

    public static final String ID = ModHelper.makePath("GigantAxeLv3");
    private static final CardStrings CARD_STRINGS = CardCrawlGame.languagePack.getCardStrings(ID); // 从游戏系统读取本地化资源
    private static final String NAME = CARD_STRINGS.NAME; // 读取本地化的名字
    private static final String IMG_PATH = "LuffyModRes/img/cards/GigantAxeLv3.png";
    ...
    public GigantAxeLv3() {
        ...
        this.magicNumber = this.baseMagicNumber = this.baseCostELA = 20;
...
//            this.upgradeMagicNumber(1);
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        AbstractDungeon.actionManager.addToBottom(
                new ApplyPowerAction(m, AbstractDungeon.player, new SuperWeakPower(m, 1), 1)
        );
        ...
        if (ModHelper.HasEnoughElasticPower(AbstractDungeon.player, CostELA)) {
            ...
            for (AbstractMonster monster : AbstractDungeon.getCurrRoom().monsters.monsters) {
                ...
                if (!monster.isDeadOrEscaped()) {
                    if(!upgraded){
                        AbstractDungeon.actionManager.addToBottom(
                                new ApplyPowerAction(monster, AbstractDungeon.player, new SuperWeakPower(monster, 1), 1)
                        );
                    }else {
                        AbstractDungeon.actionManager.addToBottom(
                                new ApplyPowerAction(monster, AbstractDungeon.player, new SuperWeakPower(monster, 2), 2)
                        );
                    }
